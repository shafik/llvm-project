...
public:
  ...
  explicit APValue(const APValue *E, unsigned N) : Kind(None), AllowConstexprUnknown(false) {
  ...
  APValue(UninitArray, unsigned InitElts, unsigned Size) : Kind(None), AllowConstexprUnknown(false) {
    MakeArray(InitElts, Size);
  }
  APValue(UninitStruct, unsigned B, unsigned M) : Kind(None), AllowConstexprUnknown(false) {
    MakeStruct(B, M);
  }
  ...
  APValue(const ValueDecl *Member, bool IsDerivedMember,
          ArrayRef<const CXXRecordDecl*> Path) : Kind(None), AllowConstexprUnknown(false) {
