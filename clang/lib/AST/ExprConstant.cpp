...
namespace {
  ...
  struct LValue {
    ...
    void set(APValue::LValueBase B, bool BInvalid = false, bool ConstexprUnknown = false) {
#ifndef NDEBUG
      ...
      IsNullPtr = false;
      AllowConstexprUnknown = ConstexprUnknown;
    }
...
static std::optional<DynamicType> ComputeDynamicType(EvalInfo &Info,
  ...
  // dynamic type.)
  if (!checkDynamicType(Info, E, This, AK, (AK == AK_TypeId ? (E->getType()->isReferenceType() ? true : false) : true)))
    return std::nullopt;
...
public:
  ...
  bool Success(const APValue &V, const Expr *E) {
    ...
    if (V.isLValue() || V.isAddrLabelDiff() || V.isIndeterminate() || V.allowConstexprUnknown()) {
      Result = V;
      return true;
    }
